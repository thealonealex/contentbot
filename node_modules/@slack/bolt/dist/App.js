    ...
    async processEvent(event) {
        ...
        if (type === helpers_1.IncomingEventType.Event) {
            ...
            if (eventListenerArgs.event.type === 'function_executed') {
                listenerArgs.complete = (0, context_1.createFunctionComplete)(context, client);
                listenerArgs.fail = (0, context_1.createFunctionFail)(context, client);
                listenerArgs.inputs = eventListenerArgs.event.inputs;
            }
            // Set sayStream() utility - only for events with channel and thread context
            const eventChannelId = (0, helpers_1.extractEventChannelId)(eventListenerArgs.event);
            if (eventChannelId !== undefined) {
                const threadTs = (0, helpers_1.extractEventThreadTs)(eventListenerArgs.event);
                const eventTs = (0, helpers_1.extractEventTs)(eventListenerArgs.event);
                const resolvedThreadTs = threadTs ?? eventTs;
                if (resolvedThreadTs !== undefined) {
                    listenerArgs.sayStream = (0, context_1.createSayStream)(client, context, eventChannelId, resolvedThreadTs);
                    listenerArgs.setStatus = (0, context_1.createSetStatus)(client, eventChannelId, resolvedThreadTs);
                }
            }
        }
        ...
        if (conversationId !== undefined && type !== helpers_1.IncomingEventType.Options) {
            listenerArgs.say = (0, context_1.createSay)(client, conversationId);
        }
        // Set respond() utility
        if (body.response_url) {
            listenerArgs.respond = (0, context_1.createRespond)(this.axios, body.response_url);
        }
            ...
            listenerArgs.respond = (0, context_1.createRespond)(this.axios, body.response_urls[0].response_url);
        }
        // Set ack() utility
        if (type !== helpers_1.IncomingEventType.Event) {
